...
public interface EmployeeRepository extends JpaRepository<Employee, Long> {
    ...
    @Query("SELECT e FROM Employee e WHERE (:name IS NULL OR e.name LIKE %:name%) AND (:department IS NULL OR e.department = :department) AND (:role IS NULL OR e.role = :role)")
